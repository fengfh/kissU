...
@11: paginal output
*/
window.dhx_globalImgPath = Global.contextPath + "/widgets/dhtmlxGrid/imgs/";
...
*   @type: public
*/
function dhtmlXGridObject(id, allowSelect){
	if (_isIE)
		try{
	...
	// zhongtao add begin
	this.excelLoader = new dtmlXMLLoaderObject(function() {
      	self.hideWaitingBox();
      	var url = self.excelLoader.xmlDoc.responseText;
        window.location.href = url;
      },
      window,true,this.no_cashe);
	// zhongtao add end
	...
	*/
	this.setSizes=function(){
		...
		if (this._ahgr){
			...
			newHeight = Math.max(newHeight,this._ahgrMin);  // zhongtao add
...
//#sorting:06042008{
	...
	*/
	this.sortField=function(ind, repeatFl, r_el){
		// zhongtao add begin
		if (!this.fldSort[ind] || this.fldSort[ind].length == 0) return false;
		// zhongtao add end
		...
		if (el.tagName == "TH"&&(this.fldSort.length-1) >= el._cellIndex
			...
			// zhongtao add begin
			this.orderBy.fieldName = this.fldSort[ind];
			this.orderBy.orderMethod = (sortType == "asc") ? "asc" : "desc";
			// zhongtao add end
			...
			//.
			//this.sortRows(el._cellIndex, this.fldSort[el._cellIndex], sortType);   // zhongtao modify
			this.refresh();//zhongtao add
			this.fldSorted=el;
...
//#config_from_xml:20092006{
	...
	*/
	_parseHead:function(xmlDoc){
		...
		this.callEvent("onParseFoot", [
		]);
		
		if (!this.multHead) {
			this.multHead = true;
			this.callEvent("onParseHead", [
			]);
		}
		//zhongtao add end	
	},
...
//#tooltips:06042008{	
	...
	*/
	_drawTooltip:function(e){
		...
		// zhongtao modifiy
		if (ced) {
			if (ced.getTitle && ced.getTitle() == false) {
				return;
			}
		...
		for (var i = 0; i < data[0].length; i++){
			...
			if (typeof data[0][i] == "object")
				...
				if (data[0][i]&&!isNaN(data[0][i])) {
					data[0][i] = this._aplNF(data[0][i],i);	
				}
				//zhongtao add end
...
//#}	
	...
	_postRowProcessing:function(r,donly){ 
		...
		// zhongtao add
		if (this.fireMenuRenderEventWhenRowCreate) {
			if (!this.callEvent("onBeforeOperationMenuPopup", [this,
							this.getRowData(r.idd),
							this.getUserData(r.idd, "menuItems")])) {
				return;
			}
		}
	},
	...
	*/			
	load:function(url, call, type){
		...
		//this.xmlLoader.loadXML(url);  // zhongtao modify
		this.xmlLoader.loadXML(url,"POST",JSON.valueOf({orderBy:this.orderBy,filterSet:this.filterSet,total:this.rowsBuffer.length}),true);//zhongtao add
	},
//#__pro_feature:21092006{		
			...
			if (r == -1){
				if (this.xmlFileUrl){
				    if (this.callEvent("onDynXLS",[i,(this._dpref?this._dpref:(max-i))]))
				        this.load(this.xmlFileUrl+getUrlSymbol(this.xmlFileUrl)+"start="+i+"&limit="+(this._dpref?this._dpref:(max-i)), this._data_type); // zhongtao modify
				}
				max=i;
				break;
			}
