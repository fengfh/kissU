...
//zhongtao add
dhtmlXGridObject.prototype.getMasterCheckbox=function(ind){
	var z=this.hdr.getElementsByTagName("input");
	for (var i=0; i<z.length; i++)
		if (z[i].type=="checkbox" && this.getFirstParentOfType(z[i],"TD")._cellIndex==ind) return z[i]
	return null;
}

//zhongtao add
dhtmlXGridObject.prototype.getMasterCheckboxState=function(ind){
	if (this.rowsCol.length == 0) {
		return false;
	}
	for (var k=0; k < this.rowsCol.length; k++) {
		if (this.rowsCol[k] && this.rowsCol[k].idd){
			var c=this.cells3(this.rowsCol[k],ind);
			if (c.isCheckbox()&&!c.isDisabled()&& c.getValue()==0) return false;
		}
	}
	return true;
}
...
//zhongtao add
dhtmlXGridObject.prototype._in_header_master_page_checkbox=function(t,i,c){
	var self=this;
	
	if (c[1].length > 0) {
		t.innerHTML="<div class='hdrcell'>"+c[0]+"<input type='checkbox' />"+c[1]+"</div>";
		
		t.firstChild.firstChild.onclick=function(e){
			self._build_m_order();
			var j=self._m_order?self._m_order[i]:i;
			var val=this.checked?1:0;
			for (var k=0; k < self.rowsCol.length; k++) {
				if (self.rowsCol[k] && self.rowsCol[k].idd){
					var c=self.cells3(self.rowsCol[k],j);
					if (c.isCheckbox()) {
						if (c.getValue() != val) {
	...
	var f=function(){
		var that = this;
		if (this._fake && i<this._fake._cCount) that=this._fake;
		that.getMasterCheckbox(i).checked=that.getMasterCheckboxState(i);
	};
	this.attachEvent("onFilterEnd",f);
	this.attachEvent("onPageChanged",f);
	this.attachEvent("onXLE",f);
	this.attachEvent("onCheck",function(id,ind,state){
		if (ind==i) f.call(this);
	})
}
