 ...
 * */
(function($){
	$.uiwidget.register("ueditor",function(selector){
		selector.each(function(){
			...
			var randomId = "ueditor_"+new Date().getTime() ; 
			setTimeout(function(){
				var ue = new UE.ui.Editor(jsonOptions);
				me.attr("ueditor_innerid",randomId ).data("ueditor",ue) ;
				ue.render(me.get(0));
			},10);
	...
	if( !$.fn.ueditor ){
		$.fn.ueditor = function(){
			var editor = $( $(this).selector+"[data-widget='ueditor']" ).data("ueditor") ;
			//var id = $(this).attr("id") ;
			//var editor = $("[ueditor_innerid='"+id+"']").data("ueditor") ;
...
(function () {
    ...
    var tmp = window.location.pathname;
        URL = $.utils.scriptPath("plugin")+"ueditor/";
    ...
    window.UEDITOR_CONFIG = {
        ...
        ,imageViewPath: URL+"jsp/process.jsp?action=view&src="

        //图片上传配置区
        ,imageUrl:URL+"jsp/process.jsp?action=imageUpload"             //图片上传提交地址
        ,imagePath:URL+"jsp/process.jsp?action=view&src="                    //图片修正地址，引用了fixedImagePath,如有特殊需求，可自行配置
        //,imageFieldName:"upfile"                   //图片数据的key,若此处修改，需要在后台对应文件修改对应参数
        //,compressSide:0                            //等比压缩的基准，确定maxImageSideLength参数的参照对象。0为按照最长边，1为按照宽度，2为按照高度
        //,maxImageSideLength:900                    //上传图片最大允许的边长，超过会自动等比缩放,不缩放就设置一个比较大的值，更多设置在image.html中

        //附件上传配置区
        ,fileUrl:URL+"jsp/process.jsp?action=fileUp"               //附件上传提交地址
        ,filePath:URL+"jsp/process.jsp?action=view&src="                  //附件修正地址，同imagePath
        //,fileFieldName:"upfile"                    //附件提交的表单名，若此处修改，需要在后台对应文件修改对应参数

         //远程抓取配置区
        //,catchRemoteImageEnable:true               //是否开启远程图片抓取,默认开启
        ,catcherUrl:URL +"jsp/process.jsp?action=getRemoteImage" //jsp/getRemoteImage.jsp"   //处理远程图片抓取的地址
        ,catcherPath:URL+"jsp/process.jsp?action=view&src="                 //图片修正地址，同imagePath
        //,catchFieldName:"upfile"                   //提交到后台远程图片uri合集，若此处修改，需要在后台对应文件修改对应参数
        //,separater:'ue_separate_ue'               //提交至后台的远程图片地址字符串分隔符
        //,localDomain:[]                            //本地顶级域名，当开启远程图片抓取时，除此之外的所有其它域名下的图片都将被抓取到本地

        //图片在线管理配置区
        ,imageManagerUrl:URL + "jsp/process.jsp?action=imageManager"  //"jsp/imageManager.jsp"       //图片在线管理的处理地址
        ,imageManagerPath:URL+"jsp/process.jsp?action=view&src="                             //图片修正地址，同imagePath

        /* */
        //屏幕截图配置区
        ,snapscreenHost: '127.0.0.1'                                  //屏幕截图的server端文件所在的网站地址或者ip，请不要加http://
        ,snapscreenServerUrl: URL +"jsp/process.jsp?action=imageUpload"  //屏幕截图的server端保存程序，UEditor的范例代码为“URL +"server/upload/php/snapImgUp.php"”
        ...
        //word转存配置区
        ,wordImageUrl:URL + "jsp/process.jsp?action=imageUpload"          //word转存提交地址
        ...
        ,toolbars:[
            ['fullscreen', 'source', '|', 
                ...
                'insertimage', 'emotion',/* 'insertvideo',*/ 'attachment', /*'map', 'gmap',*/ 'insertframe','highlightcode',/*'webapp',*/'pagebreak', '|',
