	...
	plupload.Uploader = function(settings) {
		...
		plupload.extend(this, {
			...
			init : function() {
				...
				//当前选择了几个文件
				self.bind('Error', function(up, error) {
					if($.messageBox){
						$.messageBox.error({message:error.message}) ;
					}else{
						alert( error.message ) ;
					}
				});
				...
				self.bind('FilesAdded', function(up, selected_files) {
					...
					for (i = 0; i < selected_files.length; i++) {
						...
						if( settings.max_file_num ){
							var _count = Number(settings.max_file_num) ;
							if(  up.files.length + 1 > _count  ){//文件超过设置数目
								up.trigger('Error', {
									code : plupload.FILE_NUM_ERROR,
									message : '上传文件数目不能超过'+_count+'个文件！',
									file : file
								});
								return false;
							}
						...
						if (extensionsMap && !extensionsMap[file.name.toLowerCase().split('.').slice(-1)]) {
							...
							if( settings.max_file_num === 1 ) return false ;	
							continue;
						}

						// Invalid file size
						if (file.size !== undef && (file.size > settings.max_file_size || file.size == 0 )) {
							var msg = '上传文件超过最大允许大小\n\r（当前文件大小：'+Math.floor(file.size/1024)+'KB,上传文件最大限制'+Math.floor(settings.max_file_size/1024)+'KB）！' ;
							if(file.size == 0){
								msg = "上传文件不能为空文件！" ;
							}
							up.trigger('Error', {
