	...
	plupload.runtimes.Flash = plupload.addRuntime("flash", {
		...
		init : function(uploader, callback) {
		   ...
		   if(jQuery.browser.msie)
			flashContainer.innerHTML = '<object id="' + uploader.id + '_flash" classid="clsid:d27cdb6e-ae6d-11cf-96b8-444553540000" style="outline:0" type="application/x-shockwave-flash" data="' + uploader.settings.flash_swf_url + '" ' +
			...
			uploader.bind("Flash:Init", function() {
				...
				uploader.bind("UploadFile", function(up, file) {
					...
					var params = $.extend({},settings.load_params,{name : file.target_name || file.name}) ;
					
					if( settings.upload_folder ){
						params.upload_folder = settings.upload_folder ;
					}
					
					//添加参数
					getFlashObj().uploadFile(lookup[file.id], plupload.buildUrl(settings.url, params), {
